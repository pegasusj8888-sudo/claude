...
def xt(chip):
    ...
    if 'ID46' in chip or 'ID60' in chip: return ('O','O','O')
    if 'ID4A' in chip or 'ID47' in chip or 'ID49' in chip: return ('X','△','O')
    return ('','','')
...
"- 원본 목록의 연식 범위가 실제 국내 판매와 다른 모델은 실제 기준으로 적고 비고에 적었습니다(SM3 2세대 2009~, SM6·QM6 2025 단종, 클리오는 4세대 2018~2019, 조에 2022 수입 중단, 트위지 2017~2022, 마스터 밴 2018~).",
...
"- 'XT27A/A66·XT27B·XT57B 호환'은 기아 파일 표기 규칙을 따랐습니다: ID46·ID60(4D60) → O/O/O, ID4A(Hitag AES)·ID47·ID49 → X/△/O. 실제 적용 전 장비로 재확인하세요.",
"- 출처 칸은 해당 연식 검색에서 근거로 쓴 페이지입니다. 이 환경에서는 원문 페이지를 직접 열 수 없어 검색 결과 요약을 근거로 했습니다.",
'',
'※ 색상',
"- 빨간색: 확인 불가 — 그대로 신뢰해서 작업하지 마세요(트위지).",
"- 주황색: 국내 차종 자료가 없어 닛산/르노 동일 플랫폼 기준으로 추정했거나, 자료 연식 범위 밖이거나, 출처가 상충하는 경우 — 실물 키/VIN으로 재확인.",
'',
'※ 조사하며 확인된 주요 사항',
"- SM3 1세대·CF, SM5 1·2세대, SM7 1세대(닛산 기반)는 칩을 직접 밝힌 자료를 찾지 못했습니다. 닛산 동일 플랫폼(블루버드 실피·맥시마·티아나) 기준 추정입니다.",
